...
# Pin THIS script to the ICE modem by hardware Equipment ID (IMEI).
TARGET_IMEI="${TARGET_IMEI:-868371054188863}"
...
# 1) modem id from mmcli -L, pinned by Equipment ID (IMEI)
get_modem() {
  local ids id imei

  # Robust modem id list parsing
  ids="$(mmcli -L 2>/dev/null | grep -oE 'Modem/[0-9]+' | cut -d/ -f2 || true)"

  if [[ -n "${TARGET_IMEI}" ]]; then
    for id in $ids; do
      # Robust IMEI/equipment identifier parsing using -K (stable format)
      imei="$(
      ...
      # Fallback if key missing on some setups
      if [[ -z "$imei" ]]; then
        imei="$(
  ...
  # fallback: first modem in list
  echo "$ids" | head -n1
}

# 2) bearer id from mmcli -m (take last bearer mentioned)
get_bearer() {
  mmcli -m "$1" 2>/dev/null | grep -oE 'Bearer/[0-9]+' | tail -n1 | cut -d/ -f2
}

# 3) value extraction from mmcli -b output (handles indentation + pipes)
bval() {
  local bid="$1" key="$2"
...
# ----------------------------
# MINIMAL "DO NO HARM" GUARD:
# If connectivity is healthy, exit immediately and DO NOT touch the modem.
# ----------------------------
...
while [[ -z "$MODEM" ]]; do
  ...
  # HARD GUARD: MODEM must be numeric
